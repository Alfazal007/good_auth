 ...
 */
export function auth(req: NextRequest, apiKey: string, jwtSecret: string): boolean {
    const email = req.cookies.get("email")?.value;
    const token = req.cookies.get("accessToken")?.value;
    if (!email || !token) {
        return false
    }
    return isUserAuthenticated(token, apiKey, email, jwtSecret)
}

function isUserAuthenticated(token: string, apiKey: string, email: string, jwtSecret: string): boolean {
    const [tokenValid, tokenData] = verifyToken(token, jwtSecret)
    if (!tokenValid) {
        return false
    }
...
function verifyToken(token: string, jwtSecret: string): [boolean, any] {
    try {
        const isVerifiedData = jwt.verify(token, jwtSecret) as JwtPayload
